...
$current_page = 'admin_certifications';
$page_title = t('admin.certifications') ?? 'Certificados - Admin';
...
<body>
    <div class="admin-wrapper">
        <aside class="admin-sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="certificaciones.php" class="nav-link active">
        ...
        <main class="admin-main">
            <div class="admin-header">
                <h1 class="admin-title"><?= t('admin.certifications') ?? 'Certificaciones' ?></h1>
                <button class="btn-admin btn-admin-primary" onclick="addCertification()">
                ...
                <table class="admin-table">
                    <thead>
                        <tr>
                            ...
                            <th><?= t('admin.organization') ?? 'Organización' ?></th>
                            <th><?= t('admin.date') ?? 'Fecha' ?></th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= $cert['id'] ?></td>
                            <td><?= Sanitizer::output($cert['name']) ?></td>
                            <td><?= Sanitizer::output($cert['issuing_organization']) ?></td>
                            <td><?= !empty($cert['issue_date']) ? date('d/m/Y', strtotime($cert['issue_date'])) : '-' ?></td>
                            <td>
                                <button class="btn-admin btn-admin-primary btn-admin-sm" onclick="editCertification(<?= $cert['id'] ?>)">
                                    <i class="fa-solid fa-pen"></i>
                                </button>
                                <button class="btn-admin btn-admin-danger btn-admin-sm" onclick="deleteCertification(<?= $cert['id'] ?>)">
    ...
    <script>
        function addCertification() { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>'); }
        function editCertification(id) { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>: ' + id); }
        function deleteCertification(id) { if (confirm('<?= t('admin.confirm_delete') ?? '¿Eliminar?' ?>')) { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>: ' + id); } }
    </script>
</body>
</html>
