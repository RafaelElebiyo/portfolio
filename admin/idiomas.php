...
$current_page = 'admin_languages';
$page_title = t('admin.languages') ?? 'Idiomas - Admin';
...
<head>
    ...
    <style>
        ...
        .badge-basic { background: var(--surface-raised); color: var(--text-muted); }
        .badge-intermediate { background: rgba(59, 130, 246, 0.15); color: #3b82f6; }
        .badge-advanced { background: var(--accent-subtle); color: var(--accent); }
        .badge-native { background: rgba(16, 185, 129, 0.15); color: var(--success); }
...
<body>
    <div class="admin-wrapper">
        <aside class="admin-sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="idiomas.php" class="nav-link active">
        ...
        <main class="admin-main">
            <div class="admin-header">
                <h1 class="admin-title"><?= t('admin.languages') ?? 'Idiomas' ?></h1>
                <button class="btn-admin btn-admin-primary" onclick="addLanguage()">
            ...
            <div class="admin-card">
                <table class="admin-table">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th><?= t('admin.name') ?? 'Idioma' ?></th>
                            <th><?= t('admin.level') ?? 'Nivel' ?></th>
                            <th><?= t('admin.certificate') ?? 'Certificado' ?></th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= $lang['id'] ?></td>
                            <td><?= Sanitizer::output($lang['name']) ?></td>
                            <td>
                                <span class="badge-level badge-<?= $lang['proficiency'] ?>">
                                    <?= strtoupper($lang['proficiency']) ?>
                                </span>
                                <div class="progress-bar-custom ms-2">
                                    <div class="progress-bar-fill" style="width: <?= $proficiencyMap[$lang['proficiency']] ?? '50%' ?>"></div>
                                </div>
                            </td>
                            <td><?= !empty($lang['certified_level']) ? Sanitizer::output($lang['certified_level']) : '-' ?></td>
                            <td>
                                <button class="btn-admin btn-admin-primary btn-admin-sm" onclick="editLanguage(<?= $lang['id'] ?>)">
                                    <i class="fa-solid fa-pen"></i>
                                </button>
                                <button class="btn-admin btn-admin-danger btn-admin-sm" onclick="deleteLanguage(<?= $lang['id'] ?>)">
    ...
    <script>
        function addLanguage() { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>'); }
        function editLanguage(id) { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>: ' + id); }
        function deleteLanguage(id) { if (confirm('<?= t('admin.confirm_delete') ?? '¿Eliminar?' ?>')) { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>: ' + id); } }
    </script>
</body>
</html>
