...
$current_page = 'admin_diplomas';
$page_title = t('admin.diplomas') ?? 'Diplomas - Admin';

$resumeService = new ResumeService();
$diplomas = $resumeService->getDiplomas();
...
<body>
    <div class="admin-wrapper">
        <aside class="admin-sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="diplomas.php" class="nav-link active">
        ...
        <main class="admin-main">
            <div class="admin-header">
                <h1 class="admin-title"><?= t('admin.diplomas') ?? 'Diplomas' ?></h1>
                <button class="btn-admin btn-admin-primary" onclick="addDiploma()">
            ...
            <div class="admin-card">
                <table class="admin-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= $diploma['id'] ?></td>
                            <td><?= Sanitizer::output($diploma['name']) ?></td>
                            <td><?= Sanitizer::output($diploma['issuing_organization']) ?></td>
                            <td><?= Sanitizer::output($diploma['first_year']) ?> — <?= Sanitizer::output($diploma['last_year']) ?></td>
                            <td>
                                <button class="btn-admin btn-admin-primary btn-admin-sm" onclick="editDiploma(<?= $diploma['id'] ?>)">
                                    <i class="fa-solid fa-pen"></i>
                                </button>
                                <button class="btn-admin btn-admin-danger btn-admin-sm" onclick="deleteDiploma(<?= $diploma['id'] ?>)">
    ...
    <script>
        function addDiploma() { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>'); }
        function editDiploma(id) { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>: ' + id); }
        function deleteDiploma(id) { if (confirm('<?= t('admin.confirm_delete') ?? '¿Eliminar?' ?>')) { alert('<?= t('admin.coming_soon') ?? 'Funcionalidad en desarrollo' ?>: ' + id); } }
    </script>
</body>
</html>
